...
class VTKSurface(VTKEntity3D):
    def __init__(self, vertices: np.ndarray, faces: np.ndarray, color=None, uv=None, shade=False, angle_shade=0., wireframe=False):
        self.num_vertices = 0
        self.num_faces = 0
        ...
        if wireframe:
            ...
            self.actor.GetProperty().SetColor(0.1, 0.1, 0.1)
            #wire_actor.GetProperty().SetLineWidth(1.5)
...
class VTKVisualization(object):
    ...
    def toPIL(self, background=None, size=None):
        self.window = vtk.vtkRenderWindow()
        ...
        arr = np.flipud(arr)  # Flip vertically to match PIL's coordinate system
        ...
        image = Image.fromarray(arr.astype(np.uint8), mode)
        return image
